...
export const STERScoringInterface: React.FC<STERScoringInterfaceProps> = ({
  ...
  /** Handles one-click score assignment while preserving any existing legacy notes payload. */
        ...
        <table className="w-full border-collapse text-left">
          <thead className="sticky top-0 z-10 border-y border-gray-200 bg-gray-50">
            <tr>
              ...
              {SCORE_OPTIONS.map((option) => (
                <th key={option.value} className="px-2 py-3 text-center text-xs font-bold uppercase tracking-wide text-text">
                  {option.shortLabel}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {visibleCompetencies.map((competency) => {
              ...
              return (
                <tr key={competency.id} className="border-b border-gray-100 even:bg-gray-50/40">
